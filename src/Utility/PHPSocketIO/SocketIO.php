...
namespace App\Utility\PHPSocketIO;

use Workerman\Worker;
use App\Utility\PHPSocketIO\Engine\Engine;
...
class SocketIO {
    ...
    public function __construct($port = null, $opts = array()) {
        $adapter = isset($opts['adapter']) ? $opts['adapter'] : 'App\Utility\PHPSocketIO\DefaultAdapter';
        ...
        if (!class_exists('Protocols\SocketIO')) {
            class_alias('App\Utility\PHPSocketIO\Engine\Protocols\SocketIO', 'Protocols\SocketIO');
        }
        if ($port) {
