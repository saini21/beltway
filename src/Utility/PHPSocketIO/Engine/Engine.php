...
namespace App\Utility\PHPSocketIO\Engine;

use App\Utility\PHPSocketIO\Engine\Transports\Polling;
use App\Utility\PHPSocketIO\Engine\Transports\PollingXHR;
use App\Utility\PHPSocketIO\Engine\Transports\WebSocket;
use App\Utility\PHPSocketIO\Event\Emitter;
use App\Utility\PHPSocketIO\Debug;
...
class Engine extends Emitter {
    ...
    public function checkRequest($req, $res, $fn) {
        ...
        if ($origin) {
            ...
            foreach ($allowed_origins as $allow_origin) {
                $ok = $allow_origin === $parts['scheme'] . '://' . $parts['host'] . ':' . $parts['port'] ||
    ...
    public function handshake($transport, $req) {
        ...
        if ($transport == 'websocket') {
            $transport = '\\App\\Utility\\PHPSocketIO\\Engine\\Transports\\WebSocket';
        } elseif (isset($req->_query['j'])) {
            $transport = '\\App\\Utility\\PHPSocketIO\\Engine\\Transports\\PollingJsonp';
        } else {
            $transport = '\\App\\Utility\\PHPSocketIO\\Engine\\Transports\\PollingXHR';
        }
