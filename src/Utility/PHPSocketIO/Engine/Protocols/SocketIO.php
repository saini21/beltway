...
namespace App\Utility\PHPSocketIO\Engine\Protocols;

use App\Utility\PHPSocketIO\Engine\Protocols\WebSocket;
use App\Utility\PHPSocketIO\Engine\Protocols\Http\Request;
use App\Utility\PHPSocketIO\Engine\Protocols\Http\Response;
...
class SocketIO {
    ...
    public static function input($http_buffer, $connection) {
        ...
        $connection->onClose = '\App\Utility\PHPSocketIO\Engine\Protocols\SocketIO::emitClose';
        ...
        if (!empty($connection->onRequest)) {
            ...
            // POST
            if ('\App\Utility\PHPSocketIO\Engine\Protocols\SocketIO::onData' !== $connection->onMessage) {
                $connection->onMessage = '\App\Utility\PHPSocketIO\Engine\Protocols\SocketIO::onData';
            }
            if (!$raw_body) {
                return 0;
            }
