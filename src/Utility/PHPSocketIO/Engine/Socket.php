...
class Socket extends Emitter {
    ...
    public function maybeUpgrade($transport) {
        ...
        $this->upgradeTimeoutTimer = Timer::add(
                        $this->server->upgradeTimeout, array($this, 'upgradeTimeoutCallback'), array($transport), false
        );
    ...
    public function setPingTimeout() {
        ...
        $this->pingTimeoutTimer = Timer::add(
                        $this->server->pingInterval + $this->server->pingTimeout, array($this, 'pingTimeoutCallback'), null, false);
    }
    ...
    public function flush() {
        if ('closed' !== $this->readyState && $this->transport->writable && $this->writeBuffer) {
