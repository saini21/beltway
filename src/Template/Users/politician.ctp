        ...
        <div class="form-part-one pulbox">
            ...
            <?php if (!empty($this->request->session()->read('Auth.User.profile_image')) && $this->request->session()->read('Auth.User.profile_image') != "default-user.png") { ?>
                <div class="form-group">
                    ...
                    <?= $this->element('politician-extra-details') ?>
                </div>
            ...
            <?php if ($daysOld > 90) { ?>
				...
				<input type="hidden" name="return" id="returnUrl" value="<?= SITE_URL ?>/users/politician-api/<?= $authUser['id']; ?>/">
				<input type="hidden" name="cmd" value="_s-xclick">
				<input type="hidden" name="hosted_button_id" value="VN9ZYS3GJW484">
...
">
                    <input type="hidden" name="return" id="returnUrl"
                           value="<?= SITE_URL ?>/users/politician-api/<?= $authUser['id']; ?>/">
                    <div class="form-group">
                        <div class="radio radio-danger vikas">
                            ...
                            <p><span class="icon-a"><i class="fa fa-caret-right" aria-hidden="true"></i></span> Access
                                to
                                platform<br>
                                <span class="icon-a"><i class="fa fa-caret-right" aria-hidden="true"></i></span> Monitor
                                all
                                posts<br>
...
<script type="text/javascript">
    $(function () {
        ...
        $("#phoneNumberForm").validate({
            ...
            submitHandler: function (form) {
                $.ajax({
                    url: SITE_URL + "/users/phone-number-api",
                    type: "POST",
                    data: $("#phoneNumberForm").serialize(),
                    dataType: "json",
                    success: function (response) {
                        if (response.code == 200) {
                            $('#phoneNumberSaved').fadeIn();
                        } else {
                            $().showFlashMessage("error", response.message);
                            $('#resetPasswordBtn').button('<Reset Password <em></em>');
                        }
